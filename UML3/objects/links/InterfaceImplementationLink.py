...
class InterfaceImplementationLink(AbstractLink):
    def __init__(self, can, objA, objB):
        super().__init__(can, objA, objB)
    
    def _createLine(self):
        # Ligne parente :
        super()._createLine(width = 2, dash = 16, fill = "black")
        # Points de la flèche :
        a = self._points[-2]
        b = self._points[-1]
        # Distance :
        ...
        x1 = mymap(d-l1, 0, d, a[0], b[0])
        y1 = mymap(d-l1, 0, d, a[1], b[1])
        # Idem
        l2 = 0
        x2 = mymap(d-l2, 0, d, a[0], b[0])
        y2 = mymap(d-l2, 0, d, a[1], b[1])
        ...
        self._lines.append(self._can.create_polygon(x2, y2, x3, y3, x4, y4, width = 2, fill = "white", outline = "black"))
